...
namespace _Darkland.Sources.Scripts.Unit.Stats2 {
    ...
    public class SimpleStatsHolder : StatsHolder {
        ...
        [Server]
        private void ServerSetMight(StatVal val) {
            might = val;
        }

        [Server]
        private void ServerSetConstitution(StatVal val) {
            constitution = val;
        }

        [Server]
        private void ServerSetDexterity(StatVal val) {
            dexterity = val;
        }

        [Server]
        private void ServerSetIntellect(StatVal val) {
            intellect = val;
        }

        [Server]
        private void ServerSetSoul(StatVal val) {
            soul = val;
        }

        [Server]
        private void ServerSetHealth(StatVal val) {
            health = val;
        }

        [Server]
        private void ServerSetMaxHealth(StatVal val) {
            maxHealth = val;
        }

        [Server]
        private void ServerSetHealthRegain(StatVal val) {
            healthRegain = val;
        }

        [Server]
        private void ServerSetMana(StatVal val) {
            mana = val;
        }

        [Server]
        private void ServerSetMaxMana(StatVal val) {
            maxMana = val;
        }

        [Server]
        private void ServerSetManaRegain(StatVal val) {
            manaRegain = val;
        }

        [Server]
        private void ServerSetActionPower(StatVal val) {
            actionPower = val;
        }

        [Server]
        private void ServerSetActionSpeed(StatVal val) {
            actionSpeed = val;
        }

        [Server]
        private void ServerSetMagicResistance(StatVal val) {
            magicResistance = val;
        }

        [Server]
        private void ServerSetPhysicalResistance(StatVal val) {
            physicalResistance = val;
        }

        [Server]
        private void ServerSetMovementSpeed(StatVal val) {
            movementSpeed = val;
        }

        [Client]
        private void ClientSyncMight(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.Might, val);
        }

        [Client]
        private void ClientSyncConstitution(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.Constitution, val);
        }

        [Client]
        private void ClientSyncDexterity(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.Dexterity, val);
        }

        [Client]
        private void ClientSyncIntellect(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.Intellect, val);
        }

        [Client]
        private void ClientSyncSoul(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.Soul, val);
        }

        [Client]
        private void ClientSyncHealth(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.Health, val);
        }

        [Client]
        private void ClientSyncMaxHealth(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.MaxHealth, val);
        }

        [Client]
        private void ClientSyncHealthRegain(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.HealthRegain, val);
        }

        [Client]
        private void ClientSyncMana(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.Mana, val);
        }

        [Client]
        private void ClientSyncMaxMana(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.MaxMana, val);
        }

        [Client]
        private void ClientSyncManaRegain(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.ManaRegain, val);
        }

        [Client]
        private void ClientSyncActionPower(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.ActionPower, val);
        }

        [Client]
        private void ClientSyncActionSpeed(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.ActionSpeed, val);
        }

        [Client]
        private void ClientSyncMagicResistance(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.MagicResistance, val);
        }

        [Client]
        private void ClientSyncPhysicalResistance(StatVal _, StatVal val) {
            InvokeClientChanged(StatId.PhysicalResistance, val);
        }

        [Client]
        private void ClientSyncMovementSpeed(StatVal _, StatVal val) {
