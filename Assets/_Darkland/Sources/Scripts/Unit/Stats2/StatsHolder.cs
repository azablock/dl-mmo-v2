...
namespace _Darkland.Sources.Scripts.Unit.Stats2 {
    ...
    public abstract class StatsHolder : NetworkBehaviour, IStatsHolder {
        ...
        private void Awake() {
            ...
            foreach (var darklandStat in DarklandStatsBootstrap.Init(this)) stats.Add(darklandStat);
        ...
        public Stat Stat(StatId id) {
            if (!statIds.Contains(id)) throw new ArgumentException($"StatsHolder does not contain stat of id {id}");

            return stats.First(it => it.id == id);
        }

        // [Server]
        public StatVal ValueOf(StatId id) {
            return Stat(id).Get();
        }

        // [Server]
        public Tuple<StatVal, StatVal> Values(StatId firstId, StatId secondId) {
            return Tuple.Create(ValueOf(firstId), ValueOf(secondId));
        }

        // [Server]
        public Tuple<StatVal, StatVal, StatVal> Values(Tuple<StatId, StatId, StatId> ids) {
            return Tuple.Create(ValueOf(ids.Item1), ValueOf(ids.Item2), ValueOf(ids.Item3));
        }

        // [Server]
        public Tuple<StatVal, StatVal, StatVal, StatVal> Values(Tuple<StatId, StatId, StatId, StatId> ids) {
            return Tuple.Create(ValueOf(ids.Item1), ValueOf(ids.Item2), ValueOf(ids.Item3), ValueOf(ids.Item4));
        }

        // [Server]
        public Tuple<StatVal, StatVal, StatVal, StatVal, StatVal> Values(
            Tuple<StatId, StatId, StatId, StatId, StatId> ids) {
            return Tuple.Create(ValueOf(ids.Item1), ValueOf(ids.Item2), ValueOf(ids.Item3), ValueOf(ids.Item4),
                                ValueOf(ids.Item5));
        }
        ...
        public Tuple<Stat, Stat> Stats(StatId firstId, StatId secondId) {
            return Tuple.Create(Stat(firstId), Stat(secondId));
        }
